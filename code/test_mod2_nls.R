...
min_sd <- 1
max_refit <- 6 
data_file <- 'data/ann_plant_sim2.rds'
...
for ( i in 1:nspp){
  ...
  dat$y <- rnorm( nrow(dat), dat$fecundity )
  ...
  fit1 <- nls(nls_form1, 
              data = dat, 
              start = c(l = max(dat$y), a1 = 1, a2 = 1, a3 = 1))
  ...
  fitHOI <- nls(nls_formHOI, 
                data = dat, 
                start = c(coef(fit1), b1 = 0.007, b2 = 0.02, b3 = 0.01),
                lower = 1e-30, 
                upper = c(1e2, rep(1, 6)),
                algorithm = 'port',
                control = list(warnOnly = T, tol = 1e-04, minFactor = 1e-10))
