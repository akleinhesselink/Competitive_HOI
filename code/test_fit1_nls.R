...
for ( i in 1:nspp){
  ...
  fit1 <- nls(y ~ l/(1 + a1*N1 + a2*N2 + a3*N3), data = dat, start = c(l = max(dat$y), a1 = 0, a2 = 0, a3 = 0))
  fit1 <- nls(y ~ l/(1 + a1*N1 + a2*N2 + a3*N3), data = dat , start = coef(fit1))
  dat$basic <- predict(fit1)
  
  fitHOI <- nls(y ~ l/(1 + a1*N1 + a2*N2 + a3*N3 + b1*I(N1*N2) + b2*I(N1*N3) + b3*I(N3*N2)), data = dat, start = c(coef(fit1), b1 = 0, b2 = 0, b3 = 0))
  
  fitHOI <- nls(y ~ l/(1 + a1*N1 + a2*N2 + a3*N3 + b1*I(N1*N2) + b2*I(N1*N3) + b3*I(N3*N2)), data = dat, start = coef(fitHOI))
