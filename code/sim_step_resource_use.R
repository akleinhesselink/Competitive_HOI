...
dBdu <- function(u, B, R, r, l, q, m) { B*q*f(R, r, l) - B*m }  # growth as a function of biomass and resource uptake
dRdu <- function(u, B, R, r, l) { - sum(B*f(R,r,l)) } # resource (water)

grow <- function(u, State, parms, ...){
  with(parms , {
    R  <- State[1]                             # resource first
    B  <- State[2:length(State)]               # biomass for each species
    dB <- dBdu(u, B, R, r, l, q, m)
    off <- f(R, r, l) == 0 
    dB[off] <- 0 
    dR <- dRdu(u, B, R, r, l)
    return( list( c(dR, dB))) } )
...
root <- function(u, State, parms) with(parms, { State[1] - l } )

event <- function(u, State, parms) {
  with(parms, {
    R <- State[1]
    State[ 2:length(State) ][ f(R, r, l) < 0.01 ] <- 0 
    return(State)
  })
}
...
r <- c(2.8, 2.1, 1.9) # max uptake rates mm of water per g of plant per day
l <- c(150, 90, 2)      # resource half-saturation constant, soil moisture in mm water per 500 mm soil when plant growth is half max  
...
parms <- list( r = r, l = l, m =m , p = c(rep(pulse, rainy), rep(0, times - rainy)), epsilon = epsilon, q = q, soil_m = soil_m, conversion = conversion, seedling_mass = seedling_mass, R = R, times = times)
