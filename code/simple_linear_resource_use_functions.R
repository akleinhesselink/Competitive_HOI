...
dBdu <- function(u, B, R, r, B0, q, m) { B*(q*f(R, B0, r) - m) }  # growth as a function of biomass and resource uptake
dRdu <- function(u, B, R, r, B0) { - sum( B*f(R,B0,r) ) } # resource (water)

grow <- function(u, State, parms, ...){
  with(parms , {
    R  <- State[1]                             # resource first
    B  <- State[2:length(State)]               # biomass for each species
    dB <- dBdu(u, B, R, r, B0, q, m)
    dR <- dRdu(u, B, R, r, B0)
    return( list( c(dR, dB))) } )
}

root <- function(u, State, parms) with(parms, { q*f(State[1], B0, r) - m } )

event <- function(u, State, parms) {
  with(parms, {
    R <- State[1]
    State[ 2:length(State) ][ q*f(State[1], B0, r) - m  < 0.0001 ] <- 0 
    return(State)
  })
}
...
r <- c(0.05, 0.02, 0.014) # max uptake rates mm of water per g of plant per day
B0 <- c(-6.6, -1.01, 0)      # resource half-saturation constant, soil moisture in mm water per 500 mm soil when plant growth is half max  
...
parms <- list( r = r, B0= B0, m =m , p = c(rep(pulse, rainy), rep(0, times - rainy)), epsilon = epsilon, q = q, soil_m = soil_m, conversion = conversion, seedling_mass = seedling_mass, R = R, times = times)

R <- 1:200 
par(mfrow = c(1,1))
curve( f(x, B0[1], r[1]), 0, 200)
curve( f(x, B0[2], r[2]), 0, 200, add = T)
curve( f(x, B0[3], r[3]), 0, 200, add = T)
...
test <- ode( state, times = 1:200, func = grow, parms = parms, method = 'lsode')
plot(test)

ts_plot <- plot_timeseries(test, sp_labs = c('Resource', '1', '2', '3'), mytheme = journal_theme + theme(legend.position = c(0.8, 0.5), axis.title = element_text(size = 24)))
