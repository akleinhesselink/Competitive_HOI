...
model_labs <- c('Hassel', 'HOI', 'HOI2')  


cffs <- lapply( c(fit1, fit1HOI, fit1HOI2), coef )
...
cffs_df <- 
  ...
  bind_rows(
    data.frame( Model = 'HOI', 
      ...
      gather( parameter, value, beta1:beta4))  %>% 
  filter( Model %in% c('HOI'))  %>% 
...
label_df <- 
  ...
  mutate( y_pos = ifelse(type == 'alpha', 
                         -0.1, 
                         -0.1 )) %>% 
  mutate( label = str_replace(label, '4', '123'))
...
alpha_plot %>%   
  ggsave(filename = 'figures/parameter_plot_with_3_way_HOI.png', 
         height = 5, 
         width = 8)
alpha_plot
