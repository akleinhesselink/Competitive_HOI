...
fixed_pars1 <- lapply( fit1, get_fixed_pars)
...
fit0 <- nls( 
  ...
  start = inits1, 
  lower = rep(0, 5), 
  upper = c(500, rep(6, 3), 2), 
  algorithm = 'port'
)

temp_inits <- inits1 
temp_inits$beta <- c(0, 0, 0)

fitHOI <- nls( 
  log(1/y) ~ log(model_HOI_type_00(B1, B2, B3, parms = list(lambda = lambda, alpha = alpha, beta = beta, tau = tau))), 
  data = temp_data, 
  start = temp_inits, 
  lower = rep(0, 8), 
  upper = c(400, rep(6, 6), 3),
  algorithm = 'port'
)
...
plot( 1/exp(predict(fitHOI, newdata = temp_data)), temp_data$y )
abline(0,1)

plot( 1/exp(predict(fit0, newdata = temp_data)), temp_data$y )
abline(0,1)
...
for( i in 1:3) { 
  ...
  HOI_type0_fit[[i]] <- nls( 
    log( 1/y) ~  log( model_HOI_type_0(B1, B2, B3, beta = beta, fixed_parms = fixed_pars1[[i]] )) , 
    data = temp_data,
    start = hoi_init[[i]], 
...
save( 
  ...
  fit4, 
  HOI_type1_fit, 
  HOI_type2_fit, file = 'output/model_fits2.rda'
)
