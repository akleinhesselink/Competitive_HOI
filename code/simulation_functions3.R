...
dBdu <- function(u, B, R, r, q, m, a, k) { B*(q*f(R, r, K) - m)}  # growth as a function of biomass and resource uptake
dRdu <- function(u, B, R, r, a, k, n) { - sum(B*f(R, r, K) ) }    # resource 

grow <- function(u, State, parms, ...){
  with(parms , {
    R  <- State[1]                             # resource first
    B  <- State[2:length(State)]               # biomass for each species
    dB <- dBdu(u, B, R, r, q, m, K)
    dR <- dRdu(u, B, R, r, K)
    return( list( c(dR, dB))) } )
}
