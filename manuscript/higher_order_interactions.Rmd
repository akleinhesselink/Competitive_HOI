...
Finally different species are likely to have different rates of resource uptake.  In our simulations we assume a trade-off between rate of resource uptake at high resource availability and rate of resource uptake at low resource availability. This means that species which dominate early in the season when resource availability is high will also flower and stop growing earlier in the growing season. In contrast, species that grow slower early in the growing season are able to persist later into the season when resource availability is low. 
...
We simulate a three species community with an early, middle and late season species.  Their resource uptake curves following $f_1$, $f_2$ and $f_3$ are shown in the figure below. 
...
This results in distinct growth phenology for each of the three species, with species one being the earliest and species three being the latest. 

![Example time series showing draw down of resources during the course of the simulation, upper panel, and growth of each of the species, lower panel.  Species one grows rapidly early in the season, but senesces early as well. Species three grows slowsly early in the season but grows for a longer period of time. Each species reaches its peak biomass at a different time, at which point we assume all vegetative biomass is converted to seed mass, and resource uptake by that species stops.](../figures/example_timeseries.png)

#### Response surface experiment

Using the model described above we simulated a response surface experiment where individuals of each of the three species are grown against increasing densities of one other species ('monocultures'), or grown against all possible combinations of densities of two other competitors ('bicultures').  We do not simulate the three species cases. 

After simulating all of the competitor combinations in our experiment, we then calculate the per capita reproductive output of the focal species and attempt to fit phenomenological competition models to our simulated data. 

#### Phenomenological annual plant model 

We model annual plant competition in terms of per capita reproductive output as a function of density of competitors ($n_{j,tau}$).  We use a standard functional form for phenomenological competition that has been used a  number empirical studies of annual plant competition [@kraft_plant_2015],  

\vspace{-1em}
\begin{equation} \label{eq8}
\frac{ n_{i,t+1} }{ n_{i,t} } = \lambda_{i} (1 + \alpha_{i,i} n_{i,t} + \alpha_{i,j} n_{j,t} + \alpha_{i,k} n_{k,t} )^{ \tau_i },
\end{equation}

where $\tau_{i} < 0$, $\lambda_i > 1$ and $\alpha_{ij} > 0$. In this model, $\lambda_i$ denotes maximum per capita reproductive output, $\alpha_{ij}$ is the per capita competitive effect of species $j$ on $i$ and   $\tau_i$ is a species specific parameter controlling the how steep fecundity declines with competition in general. To simplify the interpretation of the model, we assume that there is no seed mortality between years and all seeds germinate. We refer to this functional form as "Beverton-Holt". 

An alternative formulation of phenomenological competition includes competition coefficients acting as exponents on each species' density instead, 

\vspace{-1em}
\begin{equation} \label{eq9}
\frac{ n_{i,t+1} }{ n_{i,t} } = \lambda_i (1 + n_{i,t}^{\alpha_{i,i}} + n_{j,t}^{\alpha_{i,j}} + n_{k,t}^{\alpha_{i,k}})^{\tau_i}
\end{equation}

where the parameters $\alpha_{i,j}$ indicate an exponential reduction in per capita seed production of species $i$ with increased density of species $j$. We refer to this model as the "competition model version 2". 

To fit the above models we fix maximum per capita fecundity $\lambda_i$ as the value of fecundity in simulations where species $i$ is grown without competitors. We then fit the other parameters ($\alpha_{ij}$ and $\tau$) using non-linear model fitting functions, 'nls' in the R statistical program.  

#### Assessing higher order interactions

In order to assess the importance of HOIs, we re-fit the models with the structure above with HOI terms. So for the Beverton-Holt competition model we add all second order HOIs between competitors $i$ and $j$ as $\beta_{i,(ij)}N_iN_j$. We also test models with quadratic terms for each competitor $j$ as, $\gamma_{i,j}N_j^2$.  
