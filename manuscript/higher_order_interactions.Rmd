...
Example resource uptake curves for a fast growing early season species and a slower growing late season species (resource uptake fig). 
...
![Example time series showing draw down of resources during the course of the simulation, upper panel, and growth of each of the species, lower panel.  Species one grows rapidly early in the season, but senesces early as well. Species two grows slowsly early in the season but grows for a longer period of time. Each species reaches its peak biomass at a different time, at which point we assume all vegetative biomass is converted to seed mass, and resource uptake by that species stops.](../figures/example_timeseries.png)
...
where $\eta_{i} > 0$ and $\lambda_i > 0$. In this model, $\lambda_i$ denotes maximum per capita reproductive output, $\alpha_{ij}$ is the per capita competitive effect of species $j$ on $i$ and   $\eta_i$ is a species specific parameter controlling the how steep fecundity declines with competition in general. We refer to this functional form as the Beverton-Holt model. 

In order to assess the importance of HOIs, we can include various higher order terms in the basic Beverton-Holt functional form. In the two species case this includes a single interaction term between densities of species one and two and and quadratic terms for the densities of each competitor. 

\vspace{-1em}
\begin{equation} \label{eqHOI}
\frac{ n_{i,\tau+1} }{ n_{i,\tau} } = \lambda_{i}/(1 + \alpha_{i,i} n_{i,\tau} + \alpha_{i,j} n_{j,\tau} + \beta_{i,(ij)}n_in_j + \gamma_{i, i}n_i^2 + \gamma_{i, j}n_j^2)^{ \eta_i },
\end{equation}

We fit these non-linear competition models using the $\tt{nls}$ function in the statistical language R. We calculate $\lambda_{i}$ for each species as the per capita fecundity in the absence of any competitors, and set this as a fixed parameter in the model. We further constrain each of the fitted parameters to be positive. 

Results
-------
The Beverton-Holt functional form (eq. is a good fit for the effects of both inter and intraspecific competition on species 1 (fig below). Adding HOI terms does not appear to improve the model fit for species one. In contrast, the basic does not fit the response of species two as well but adding HOI terms improves the model fit. 

![Simulated per capita seed production of each species in response to increasing intraspecific density.  Colored lines show three different levels of interspecific density. Left side shows the response of species 1 and the right shows the response of species 2. The dashed line shows the best fit line from the Beverton Holt model and the dotted line shows the best fit from the HOI model described above includeing quadratic terms ](../figures/both_fits.png)

Examining the fitted parameters for each of the models we can clearly see the importance of the HOI terms for the competitive effects on species two. 

```{r echo = FALSE, results = 'asis'}

library(knitr)
load( '../data/fits_df.rda')
kable(fits_df, digits = 3, caption = 'Fitted competition parameters and residiual squared error for the basic model and the model containing higher order interaction terms')

```

Including the HOI terms improves the residual squared error for both species one and two in a similar amount.  However, we can see that the estimated HOI term for species two, $\beta$, is substantially greater than the estimated pairwise competition coefficients $\alpha$. We can also see that adding the HOI terms changes the estimates for the other terms in the model. In the basic model, species one and species two appear to have stronger intraspecific competition parameters than interspecific.  But for species two, the estimates for the $\alpha's$ change greatly in the model containing HOI's and in fact the estimated intraspecific effect goes to zero (keeping in mind that the parameters are constrained to be $>0$). 

Discussion
-----------

It is clear from the above example that a relatively simple resource consumption model may require HOI terms in order to be approximated by a phenomenological model.  However, this is not simply a case of additional higher order terms making for a better appriximation of a non-linear function. In fact, we can approximate the effects of competition on species one quite well without resorting to HOIs. It is only for species two that we need to include HOIs to approximate the effects of competition. 

We believe this has to do with the temporal nature of competition in this system. While species one is active, species two is also always active. The effects of initial competitor density on species one are relatively simple and the Beverton-Holt model does a good job approximating the effects of competition. 

However, for species two the competitive environment can be split up into two segments: the first part of the growing season over which it interacts with species one, from time $t=0$ up until time $t=z$, and the part of the growing season where it only interacts with itself, time $t=z$ until time $t=f$. Approximating its response to initial competitor density requires 

However the amount that species two is able to grow on these resources depends on its total biomass at this point which depends on the initial density of both species and the the intraspecific effects of two on itself. 



Predicting the biomass of species two at this point and thus its total growth over the course of the season requires thinking through a rather convoluted set of direct and indirect effects.  First the size of species two at the point when species one flowers, requires knowing the starting density of species one and two. This is equivalent to basic pairwise effects and in fact we can fit basic pairwise effects up to this point (new fig).

From this point on the s growth of species two after one flowers depends on the size it reaches at this point. The longer it takes to flower the larger species two is able to grow by the time of flowering. This in turn will increase its net growth rate through the second part of the season. Thus we see a negative HOI in this model. In other words, increasing the initial density of species two has a greater competitive effect when species one is also present. 

This system is characterized by a seasonal pulse of resource availability and the lack of an equilibrium between species biomass and resource concentrations in the environment.  In contrast, the competitive dynamics of resource mechanistic models can be reduced to linear phenomenological models at equilibrium, a technique that has been widely used to understand the viability and stability of competitive coexistence (cite Meszena, Kleinhesselink, Tilman).  

We believe that it is likely that many ecological systems are
However, if iterated across many seasons, the abundances of species one and two in the model above will reach some kind of equilibrium, either competitive exclusion or coexistence. 
